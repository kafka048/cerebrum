...
def score_burnout(signals: SignalResult) -> ProfileResult:
    ...
    if consistency.average_positive_run > POSITIVE_RUN_THRESHOLD:
        score += 2
        evidence.append("Long execution blocks confirmed")

    if consistency.average_negative_run > NEGATIVE_RUN_THRESHOLD:
        score += 1
    ...
    if streak.longest_streak > LONGEST_STREAK_THRESHOLD:
        score += 1
        evidence.append(
